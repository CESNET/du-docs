...
Harbor používá funkčnost jednotného přihlašování e-infra, takže se můžete snadno přihlásit do této služby se svým účtem Masarykovy univerzity pomocí zobrazeného tlačítka pro přihlášení.
...
Ve vašem uživatelském profilu uvidíte položku nazvanou "CLI secret", což je heslo, které použijete k přihlášení do svého účtu přes příkazový řádek. Toto heslo můžete zkopírovat kliknutím na ikonu kopírování vedle něj.
...
Ve stejné složce, kde máte svůj Dockerfile, spusťte následující příkaz: `docker build -t cerit.io/$USER/tutorial:v0.1 .` (nahraďte `$USER` svým uživatelským jménem v Harboru, bez velkých písmen) (vyberte `cerit.io` nebo `hub.cerit.io` na začátku názvu obrazu podle toho, které jméno jste použili v kroku `docker login`)
...
Nyní vytvořte a nahrajte obraz, pouze změňte `:v0.1` na `:v0.2` na konci názvu obrazu.

Pokud jste úspěšně nahráli svůj obraz, měli byste ho vidět ve svém projektu takto:

![pushed](/img/ceritsc/harbor/pushed.png)

## Správa vašeho repozitáře (Docker obraz)

Když kliknete na název vašeho repozitáře, uvidíte všechny artefakty
spojené s repozitářem (tyto představují různé verze
vašeho obrazu)

![artifacts](/img/ceritsc/harbor/artifacts.png)

Na této stránce můžete vybrat artefakt a např. zkopírovat ho do jiného
repozitáře nebo ho smazat, abyste uvolnili místo (obě tyto operace
jsou dostupné prostřednictvím tlačítka `ACTIONS`).

Vezměte prosím na vědomí, že smazáním artefaktu nemusíte nutně uvolnit
veškeré místo, které uvádí v `Size`, artefakty mohou sdílet takzvané `layers`. V našem případě jsme vytvořili 2 verze obrazu, které se liší pouze v posledním příkazu
Dockerfile. Všechno, co bylo provedeno před posledním příkazem, je sdíleno
mezi 2 artefakty, takže smazáním jednoho nebo druhého uvolníme pouze
několik kilobajtů místa.

Také si prosím uvědomte, že smazání artefaktu okamžitě neuvolní místo,
určení, zda je `layer` odkazován nějakým artefaktem, je
nákladná operace a provádí se pouze jednou denně o půlnoci.

Kliknutím na název artefaktu můžeme vidět podrobnější informace
(např. architektura, konfigurace a historie sestavení). V tomto přehledu můžeme také skenovat na zranitelnosti.

![vulnerabilities](/img/ceritsc/harbor/vulnerabilities.png)

Skenování zranitelností určí, zda náš obraz obsahuje nějaké známé
zranitelnosti a zda je můžeme opravit změnou našeho základního obrazu (obrazu v příkazu `FROM` v Dockerfile). Můj sken na zranitelnosti vygeneroval následující výsledky:

![scan](/img/ceritsc/harbor/scan.png)

Můžeme vidět, že v mém obrazu jsou některé zranitelnosti s nízkou a
střední závažností. Také bych mohl opravit jednu ze zranitelností použitím
...
chceme, můžeme přidat nějaké informace o obrazu. To může být užitečné v
veřejném projektu, aby lidé mohli říct, co obraz vlastně dělá. 
Informace jsou formátovány pomocí [markdown jazyka](https://www.markdownguide.org/).
