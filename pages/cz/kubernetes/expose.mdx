...
V podstatě existují dva typy exponovaných aplikací: *webové* aplikace a *ostatní* aplikace. Rozdíl mezi těmito typy spočívá v požadované IP adrese. Webová aplikace sdílí IP adresu s jinými webovými aplikacemi, zatímco ostatní aplikace potřebují novou IP adresu pro každou službu. Počet IP adres je omezený, proto použijte webový přístup, pokud je to možné.

V následující dokumentaci jsou fragmenty YAML, které mají být nasazeny pomocí `kubectl` takto: `kubectl create -f file.yaml -n namespace`, kde `file.yaml` obsahuje níže uvedené YAML a `namespace` je prostor názvů obsahující aplikaci. Tato část dokumentace také předpokládá, že aplikace (nasazení) již existuje. Pokud si nejste jisti, přečtěte si znovu [hello example](../examples/helloworld).
...
`CNAME` musí být předem nastaven a propagován, aby služba Let's Encrypt mohla ověřit doménu, když je požadováno TLS. To znamená, že bezproblémová migrace z jakéhokoli systému na naši infrastrukturu není snadno možná.
...
```
2. Vylistujte události, které se dějí ve vaší namespace a zjistěte, zda je tam varování o kvótách (`kubectl get events -n [your_namespace]` bude obsahovat nějaké řádky s `exceeded quota`)
```bash
...
Je také možné vystavit aplikaci na soukromé MUNI IP. V tomto případě bude aplikace dostupná z MUNI sítě nebo pomocí MUNI VPN. Tento typ vystavení je vybrán pomocí anotace `metallb.universe.tf/address-pool: privmuni`. Tento případ je preferován, protože nepoužívá veřejnou IP. Úplný příklad je níže.
