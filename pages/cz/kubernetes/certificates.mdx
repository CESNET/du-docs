...
* Vydavatel `letsencrypt-prod` je pouze pro web a automaticky kontroluje web na zadaném doménovém jménu během procesu vydávání. Je automaticky používán při vystavování aplikace prostřednictvím [ingress](/kubernetes/expose#web-based-applications).
* Vydavatel `letsencrypt-prod-dns` je určen pro jiné aplikace. Rozdíl je v kontrole dns. Kontrola dns je však pomalejší, protože změna dns se musí nejprve propagovat.
...
Konfigurace generuje tajemství, specifikované v `spec.secretName`, obsahující certifikát a pár soukromých klíčů.
