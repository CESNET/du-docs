...
<TipWarning>Cleaning must be done within a single **RUN** command.</TipWarning>
...
* For Kubernetes, there is no need to use the `EXPOSE` command to publish network ports. See [Exposing Applications](../kubernetes/kubectl-expose).
...
Automated build can be done using a CI/CD pipeline. The build is done using the *runner* component which is part of many github/gitlab installations. You can use [gitlab.ics.muni.cz](https://gitlab.ics.muni.cz), see [GitLab Containers](../gitops/containers_build) how to do it. This gitlab also provides docker registry, so once built image can be stored here for use from Kubernetes. The registry is public or private depending on the project settings, see [GitLab Containers](../gitops/containers_build#how-to-use-images-from-gitlab-registry).
