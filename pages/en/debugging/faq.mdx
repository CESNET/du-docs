...
**Answer:** The deployment is being made to a *Namespace* that is running out of quota or even has quota set to zero. You need to increase the *Namespace* quota, which may require decreasing the quotas of your other *Namespaces* or requesting an explicit *Project*. See [Projects, Namespaces, and Quotas](../rancher/quotas).
...
**Answer:** HTML code 413 means entity too larger. Helm stores whole deployment and values into a `Secret` object. Limit of the Secret size is about 1.5MB. Verify, if there is no big file in the whole chart.
