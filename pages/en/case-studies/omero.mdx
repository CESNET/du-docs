...
The second option starts with database and it seems to be the Postgres database. Here, we are lucky because Kubernetes provide a Postgres operator for easy database deployment, see our [Operators](../operators/postgres-cnpg) documentation.
...
The `metadata.name` can be arbitrary but will be referenced in subsequent manifests. [Link](/examples/ceritsc/manifests/db-app-secret.yaml).
...
The cluster create has only a single Pod, it is not a real and resilient cluster. For high availability clusters, see [Postgres Operator](../operators/postgres-cnpg).
...
Finally, we are ready to deploy the `omero-server` using the [complete manifest](/examples/ceritsc/manifests/omero-server.yaml). It also exposes two ports in the same way as the Docker command. **Do not forget to set your own ROOT password in the following fragment.** Also, rather a Secret object should be used to pass the password.
...
[Link](/examples/ceritsc/manifests/omero-server-svc.yaml).
...
Now, we are able to deploy the `omero-web` [manifest](/examples/ceritsc/manifests/omero-web.yaml).
...
[Link](/examples/ceritsc/manifests/omero-web-svc.yaml).
...
[Link](/examples/ceritsc/manifests/omero-web-ingress.yaml).
