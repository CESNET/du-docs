...
* `source` `space` &rarr; `source` `volume` (one way) -- Data is cloned to the volume at startup.
* `project` `space` &harr; `project` `volume` (both ways) -- This synchronization contains 3 steps:
