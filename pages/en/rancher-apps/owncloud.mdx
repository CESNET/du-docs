...
* Important prerequisite: application pod needs to use [PVC](../kubernetes/pvc) in order to ownCloud client can sync data into the pod. If no PVC is used, no sync is possible.
