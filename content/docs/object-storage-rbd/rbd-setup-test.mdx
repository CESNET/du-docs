---
title: LUG - Connecting and Configuring Ceph RBD Using a Linux Client
---
...
<Callout type="info" emoji="ℹ️">
Rolling back an image to a snapshot means overwriting the current version of the image with data from a snapshot. The time it takes to execute a rollback increases with the size of the image. It is faster to clone from a snapshot than to rollback an image to a snapshot, and is the preferred method of returning to a pre-existing state. For more information see [documentation](https://docs.ceph.com/en/pacific/rbd/rbd-snapshot/).
</Callout>
...
Be aware: some filesystems require the use of additional options for the `mount` command — e.g. XFS:
